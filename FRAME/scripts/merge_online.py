...
from utils_frame import visualize, pcd_publisher, RobotFeatures
import os
...
class Node:
	...
	def merge(self) -> List[List[float]]:
		...
		while np.shape(np.asarray(self.r1.map.points))[0] == 0 and np.shape(np.asarray(self.r2.map.points))[0] == 0:
			print("Waiting to receive maps...")
		...
		## Print size of maps
		print("Number of points in r1/map: %.f" % np.shape(np.asarray(self.r1.map.points))[0])
		print("Number of points in r2/map: %.f" % np.shape(np.asarray(self.r2.map.points))[0])
		...
		## Log index info
		print("Min indexes info: " + str(argmin) + ", " + str(argmin_index))
		...
		toc = time.time()
		## Log time info
		print("Sampling took %.3f seconds \n" % (toc - tic))
		...
		print("Estimated initial transform: \n{} \n".format(self.T))
...
if __name__ == '__main__':
	...
	## Create object
	robot1 = RobotFeatures(m1_topic, from_file=True, namespace=r1_ns)
	robot2 = RobotFeatures(m2_topic, from_file=True, namespace=r2_ns)
